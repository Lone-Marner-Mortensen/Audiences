...
object Members {
    ...
    val members: RDD[MemberDetails] = sc.textFile(path).map(MemberDetails(_,date,addOrRemove))//Won't fail because of sparks lazy evaluation.
     ...
     Try(members.isEmpty) match {
      ...
      case Failure(e) => if(e.isInstanceOf[InvalidInputException])
                            sc.emptyRDD[MemberDetails]//Should return empty rdd, if there are no files for given day.
                         else
                            throw e
  ...
  private[audience] def getMemberDetails(dates:List[String],audienceId:Int)(implicit basePath:String, sc:SparkContext): RDD[MemberDetails] = {
    ...
    val rdds = dates.map{ d => getMemberDetails(d,audienceId).union(getMemberDetails(d,(-1)*audienceId)) }
    sc.union(sc.emptyRDD[MemberDetails]::rdds)//Here we handle the case where dates = List().
  }
    ...
    if (memberDetails.isEmpty)
      ...
      memberDetails
        ...
        .flatMap { case (_, mdIterator) =>
          ...
          val containsAddition = md.filter{ m => m.date == mdLastDay.date && m.addOrRemove == Addition }.nonEmpty
